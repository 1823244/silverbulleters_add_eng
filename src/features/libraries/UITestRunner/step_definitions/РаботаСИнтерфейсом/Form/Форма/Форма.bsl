...
&НаКлиенте
Перем КоличествоПопытокПоискаОкон;
	...
	Иначе	
		...
		Если Не ЭтоМодальныйДиалогПлатформы Тогда
			Если АктивноеОкно.Заголовок <> АктивноеОкно2.Заголовок Тогда
				Если КоличествоПопытокПоискаОкон = 0 Тогда
					ТестоваяПеременная = КонтекстСохраняемый.ТестовоеПриложение.НайтиОбъект(Тип("ТестируемоеОкноКлиентскогоПриложения"), Строка(Новый УникальныйИдентификатор), ,2);
					КоличествоПопытокПоискаОкон = 1;
					ОткрылосьОкно(ИмяОкна,ЧастьИмени2,ЧастьИмени3, ЧастьИмени4, ЧастьИмени5, ЧастьИмени6,ЧастьИмени7);
					Возврат;
				КонецЕсли;					
...
Функция ПолучитьОшибкуПерехода(ИмяТЧ,ОшибкаПереходВниз,ОшибкаПереходВверх,ТабПарам)
	...
	Для Каждого Элем Из Колонки Цикл
		...
		ЗначениеКолонки  = ОписаниеСтроки[Элем.Значение];
...
Процедура ЯБудуВыбиратьВнешнийФайл(Знач ИмяФайла) Экспорт
	...
	Если СтрНайти(ИмяФайла,"$") <> 0  Тогда //есть переменные
		ИмяФайла = Ванесса.ВычислитьВнутреннееВыражение(ИмяФайла);	
	КонецЕсли;
...
Процедура ТабличныйДокументСодержитЗначенияОбластей(ИмяЭлементаФормы,ТабПарам) Экспорт
	...
	Если МассивРасхождений.Количество() > 0 Тогда
		...
		ТекстСообщения = Вычислить("СтрСоединить_(МассивРасхождений, Символы.ПС)");
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
...
Процедура ТабличныйДокументСодержитЗначенияОбластейПоШаблону(ИмяЭлементаФормы,ТабПарам) Экспорт
	...
	Если МассивРасхождений.Количество() > 0 Тогда
		...
		ТекстСообщения = СтрЗаменить(ТекстСообщения,"%1",ИмяЭлементаФормы); 
		МассивРасхождений.Вставить(0, ТекстСообщения);
		ТекстСообщения = СтрСоединить_(МассивРасхождений, Символы.ПС);
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
